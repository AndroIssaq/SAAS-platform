...
export function ContractHTMLGenerator({
  ...
  const handleDownloadWord = () => {
    ...
    const htmlContent = generateProfessionalContractHTML(contractData)

    // Add Word-specific XML namespace for better compatibility
    const wordHTML = `
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `عقد-${contractNumber}.doc`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)

    toast.success('تم تحميل العقد بصيغة Word')
  }
